...
Let's take a pretty straightforward VPN setup, where a on-premise database replicates its contents to an RDS instance:
![Simple VPN](/assets/posts/2018-08-31-Testing-AWS-VPN-connections-with-EC2/simple-vpn.png)

You're responsible for setting up the AWS side, and you're working with the IT department that manages the corporate data center. The remote engineer indicates that he can't connect to the RDS instance, but there is no way for you to log in and check their configuration for them. In this case, simulating the connection might help you determine if the problem is located at the AWS side, or in their data center.

To achieve this, we will setup a separate EC2 instance functioning as a Customer Gateway. We will use Openswan to set up a site-to-site VPN connection with the existing VPC VPN. The instance will be placed in a separate VPC to guarantee complete isolation of the other AWS resources.
...
Now that we have an instance that will function as our Customer Gateway, we can configure it in our RDS VPC. Switch back to that VPC and navigate to the "Customer Gateways" section. Click "Create Customer Gateway" and fill in a name and the public IP address of the EC2 instance we've just created.

![New Customer Gateway](/assets/posts/2018-08-31-Testing-AWS-VPN-connections-with-EC2/create-customer-gateway.png)

If you had not done so before, setup a Virtual Private Gateway and attach it to your RDS VPC.
...
Now open the configuration file you've downloaded in the previous chapter and follow its instructions. The first step is to open `/etc/sysctl.conf` and add a few lines. In my experience, the official instructions are missing two lines, so the content I used it this:
```
...
Follow the instructions for creating a file called `/etc/ipsec.d/aws.secrets`.
...
1. If things work from your new VPN, you can at least guarantee that what you're trying to achieve is generally possible
